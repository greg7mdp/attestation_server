...
std::enable_if_t<
    ...
    ustring_view> static inline to_ustring_view(std::array<T, N> const& a)
{
...
std::enable_if_t<
    std::is_same<T, char>::value || std::is_same<T, unsigned char>::value,
    ustring_view> static inline to_ustring_view(std::vector<T, Alloc> const& v)
{
...
private:
    static_assert((num_bits % 32) == 0, "The length of a base_uint in bits must be a multiple of 32.");

    static_assert(num_bits >= 64, "The length of a base_uint in bits must be at least 64.");
    ...
    constexpr std::optional<decltype(data_)> parseFromStringView(ustring_view sv) noexcept
    {
...
template <std::size_t Bits, class Tag>
inline bool operator==(base_uint<Bits, Tag> const& a, base_uint<Bits, Tag> const& b)
{
...
static inline Currency const& noCurrency()
{
...
static inline Currency const& badCurrency()
{
...
[[noreturn]] inline void Throw(Args&&... args)
{
    ...
    LogThrow(std::string("Throwing exception of type " + type_name<E>() + ": ") + e.what());
    throw e;
}
...
std::enable_if_t<
    std::is_same<T, char>::value || std::is_same<T, unsigned char>::value,
    void> static inline secure_erase(std::array<T, N>& a)
{
