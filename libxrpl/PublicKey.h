...
inline ustring toBase58(TokenType type, PublicKey const& pk)
{
    return encodeBase58Token(type, pk.view());
}

template <>
std::optional<PublicKey> parseBase58(TokenType type, ustring_view sv);
...
*/
/** @{ */
[[nodiscard]] std::optional<KeyType> publicKeyType(ustring_view slice);

[[nodiscard]] inline std::optional<KeyType> publicKeyType(PublicKey const& publicKey)
{
...
*/
[[nodiscard]] bool
verify(PublicKey const& publicKey, ustring_view m, ustring_view sig, bool mustBeFullyCanonical = true) noexcept;
